  ...
  try {
    ...
    if (editId) {
      ...
      const updateData = {
        ...
        typeOfFood: Array.isArray(formData.typeOfFood)
  ? formData.typeOfFood
  : [formData.typeOfFood],
      ...
      result = await updateRestaurant(editId, updateData);
      ...
      result = await addRestaurant(data);
    }
    ...
    console.log("ERROR:", err.response?.data); // 👈 ye zaroor dekhna
    ...
    <div className="max-w-7xl mx-auto space-y-8 p-6 bg-white min-h-screen">
      ...
      <div className="flex justify-between items-end border-b border-slate-100 pb-10">
        ...
        <Button
          ...
          className="bg-black text-white px-8 py-4 rounded-xl text-xs font-bold uppercase shadow-lg"
        >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {restaurantList.map((rest) => (
            <div
              key={rest._id}
              className="bg-white rounded-4xl border border-slate-100 shadow-sm overflow-hidden relative group hover:shadow-xl transition-all duration-500"
            >
              <div className="absolute top-4 right-4 z-20 flex gap-2">
                <button
                  onClick={() => handleEditOpen(rest)}
                  className="p-2.5 bg-white/90 backdrop-blur text-black rounded-xl border border-slate-100 hover:bg-black hover:text-white transition-all"
                >
                  <Edit3 size={16} />
                </button>
                <button
                  onClick={() => handleDelete(rest._id)}
                  className="p-2.5 bg-white/90 backdrop-blur text-red-600 rounded-xl border border-slate-100 hover:bg-red-600 hover:text-white transition-all"
                >
                  <Trash2 size={16} />
                </button>
              </div>
              ...
              <div className="h-52 bg-slate-50 relative overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4">
                  <span className="px-3 py-1 bg-white text-black rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm border border-slate-100">
                    {rest.venue}
                  </span>
                </div>
              </div>
              ...
              <div className="p-8">
                <h4 className="text-xl font-black text-slate-900 uppercase truncate mb-1">
                  {rest.businessName}
                </h4>
                <div className="space-y-2 mb-6">
                  <div className="flex items-center gap-1 text-slate-400">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-wider truncate">
                      {rest.address?.cityState || "No address"}
                    </p>
                  </div>
                ...
                <div className="pt-6 border-t border-slate-50 flex justify-between items-center">
                  <div>
                    <span className="text-[9px] uppercase font-black text-slate-300 block mb-1">
                      Cuisine
                    </span>
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            <div className="p-8 flex justify-between items-center border-b border-slate-50">
              <h3 className="text-slate-900 text-xl font-black uppercase tracking-widest">
                {editId ? "Update Venue" : "New Venue"}
              </h3>
              <button
                onClick={() => setIsModalOpen(false)}
                className="bg-slate-50 p-2 rounded-full hover:bg-slate-100 transition-colors"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-10 space-y-6 overflow-y-auto custom-scrollbar">
              <div className="flex gap-3">
                {["restaurant", "cafe"].map((type) => (
                  <button
                    ...
                    className={`flex-1 py-4 px-6 rounded-2xl font-black transition-all border text-[11px] uppercase tracking-widest flex items-center justify-center gap-3 ${
                      formData.venue === type
              ...
              <div
                ...
                className="h-44 w-full rounded-3xl border-2 border-dashed border-slate-100 bg-slate-50/50 flex items-center justify-center cursor-pointer overflow-hidden relative group"
              >
                {preview ? (
              ...
              <div className="flex gap-4 pt-6">
                ...
                <Button
                  onClick={handleSave}
                  disabled={saving}
                  className="flex-2 bg-black text-white font-black uppercase tracking-widest rounded-2xl shadow-xl"
                >
                  {saving ? (
